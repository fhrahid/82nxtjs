...
import { SHIFT_MAP } from '@/lib/constants'; // ADDED (mapping codes -> timings / labels)
...
export default function ShiftView({ open, onClose, roster, headers }: Props) {
  ...
  const teamList: string[] = useMemo(()=> roster?.teams ? Object.keys(roster.teams) : [], [roster]);
  ...
  const filteredEmployees = useMemo(()=>{
    ...
    Object.entries(roster.teams).forEach(([teamName, emps]:[string, any])=>{
      ...
      (emps as any[]).forEach(emp=>{
        ...
        if (selectedShifts.length && !selectedShifts.includes(shift)) return;
        out.push({
          name: emp.name,
          id: emp.id,
          team: teamName,
          shift // keep original code for filtering; display will map
        });
      });
    });
    return out;
  },[selectedDate, roster, headers, selectedShifts, selectedTeams]);
  ...
  // Minimal helper to translate code → timing / label
  ...
  return (
    <div className="modal-overlay sv-fullwidth">
          ...
          <div className="sv-content-col-wide">
              ...
              <div className="sv-chip-row-wide">
                {shiftCodes.map(code=>(
                  <button
                    ...
                    onClick={()=>toggle(selectedShifts,code,setSelectedShifts)}
                    title={`${code} → ${displayShift(code)}`}
                  >{code}</button>
                ))}
                <button className="sv-chip-wide clear" onClick={()=>setSelectedShifts([])}>Clear</button>
              </div>
            </div>
            ...
            <div className="sv-filter-block-wide">
              ...
              <div className="sv-chip-row-wide">
                ...
                <button className="sv-chip-wide clear" onClick={()=>setSelectedTeams([])}>Clear</button>
              </div>
            </div>
      ...
      {/* Original styles left untouched below */}
      <style jsx global>{`
