...
export default function MonthCompactCalendar({
  ...
  return (
    <div className="mc-month-calendar large">
      ...
      <style jsx>{`
        .mc-month-calendar.large { background:#121b24; border:1px solid #2d3d4c; border-radius:14px; padding:14px 16px 18px; width:100%; box-sizing:border-box; display:flex; flex-direction:column; gap:10px; }
        .mc-header-row { display:flex; justify-content:center; align-items:center; gap:12px; }
        .mc-month-title { font-size:.9rem; letter-spacing:1px; font-weight:600; color:#e1e8ef; text-transform:uppercase; }
        .mc-nav-btn { background:#1b2833; border:1px solid #314252; border-radius:6px; color:#93a7ba; padding:6px 12px; font-size:1rem; cursor:pointer; transition:.18s; font-weight:600; }
        .mc-nav-btn:hover:not(:disabled) { background:#285072; border-color:#3d6a8d; color:#fff; }
        .mc-nav-btn:disabled { opacity:.3; cursor:not-allowed; }
        .mc-weekdays { display:grid; grid-template-columns:repeat(7,1fr); gap:6px; }
        .mc-weekday { text-align:center; font-size:.65rem; letter-spacing:.5px; color:#93a7ba; padding:2px 0; font-weight:500; }
        .mc-weeks { display:flex; flex-direction:column; gap:6px; }
        .mc-week { display:grid; grid-template-columns:repeat(7,1fr); gap:6px; }
        .mc-day { position:relative; background:#1b2833; border:1px solid #314252; border-radius:8px; height:52px; font-size:.8rem; color:#6f879d; display:flex; align-items:center; justify-content:center; cursor:pointer; transition:.18s; padding:0; font-weight:500; }
        .mc-day.active { color:#d5e1ec; }
        .mc-day:hover.active { background:#285072; border-color:#3d6a8d; color:#fff; }
        .mc-day.selected { background:#3b6ea7; border-color:#74a8e7; color:#fff; font-weight:700; box-shadow:0 0 0 2px rgba(90,140,200,.25); }
